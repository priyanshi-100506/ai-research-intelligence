...
# Genuine configuration-driven data targets
TARGET_SOURCES = [
    {
        "name": "Google Developers Blog",
        "url": "https://feeds.feedburner.com/GCPCloudBlog",
        "category": "Google Tech & AI",
        "parser_type": "rss",
...
def run_ingestion_pipeline():
    ...
    try:
        ...
        # ======================================================================
        # PHASE 1: CONFIG-DRIVEN INGESTION
        # ======================================================================
        ...
        for source in TARGET_SOURCES:
            ...
            try:
                if source["parser_type"] == "rss":
                    # The worker now accepts the mapped metadata target URL context
                ...
                logging.error(f"Defensive isolation triggered. Skipping {source['name']}: {str(e)}")

        # ======================================================================
        # PHASE 2: BATCHED DEDUPLICATION & MULTIMEDIA EXTRACTS
        # ======================================================================
        ...
        for item in all_items:
            ...
            # Late-bound execution: load audio transcripts ONLY if record is net-new
            if item.source_id.startswith('@') and not content_body:
                logging.info(f" -> Fetching full transcript for: '{item.title[:40]}...'")
                try:
                    ...
                    logging.error(f"Transcript fetch skipped for {item.article_id}: {str(tx_err)}")
                    continue
        ...
        # Optimized Batch Ingestion Transaction
        if pending_scraped_records:
            try:
                db.add_all(pending_scraped_records)
                db.commit()
                new_scraped_entries = len(pending_scraped_records)
                logging.info(f"Successfully saved {new_scraped_entries} net-new rows to database state.")
        ...
        # ======================================================================
        # PHASE 3: AUTOMATED AI CURATION BATCHING
        # ======================================================================
        ...
        for article in uncurated_records:
            logging.info(f" -> Submitting schema contract to Gemini for: '{article.title[:40]}...'")
            try:
        ...
        # Optimized Batch Curation Warehouse Transaction
        if pending_curation_records:
            try:
                db.add_all(pending_curation_records)
                db.commit()
                logging.info(f"Committed {len(pending_curation_records)} AI analytical records safely.")
            except Exception as batch_cur_err:
                db.rollback()
                logging.error(f"Warehouse transaction batch commit error: {str(batch_cur_err)}")

        # ======================================================================
        # PHASE 4: DISPATCH EVALUATION MATRIX (DUE DILIGENCE INBOX SAFETY)
        # ======================================================================
        logging.info("Evaluating message delivery conditions...")
        time_threshold = datetime.utcnow() - timedelta(hours=LOOKBACK_HOURS)
        
        has_recent_articles = db.query(CuratedArticle).filter(
            CuratedArticle.created_at >= time_threshold
        ).first() is not None

        if (new_scraped_entries > 0) or has_recent_articles:
            logging.info("Fresh engineering intelligence verified. Dispatching digest presentation layers...")
            try:
                mailer.send_daily_briefing(recipient_email=MY_INBOX)
                logging.info("Briefing package successfully pushed to transport layer gateway.")
            except Exception as mail_err:
                logging.error(f"Notification infrastructure gateway failure: {str(mail_err)}")
        else:
            # 🔄 FALLBACK GATEWAY: Send an explicit pulse alert instead of exiting silently
            logging.info("Pipeline idle: No new streaming delta. Dispatching system state pulse alert...")
            ...
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #e1e4e6; padding: 20px; border-radius: 6px;">
                ...
                <p style="color: #4a5568; line-height: 1.5;">Your database deduplication layer rejected all incoming streams today because target technology streams haven't published fresh content within the lookback window.</p>
                <hr style="border: 0; border-top: 1px solid #e1e4e6; margin: 20px 0;"/>
                <h3 style="color: #2d3748;">🎯 High-Signal Fallback Matrices:</h3>
                <p style="color: #4a5568; margin-bottom: 8px;">While the automated pipeline rests, here are your core authenticated engineering feeds to review manually:</p>
                <ul style="padding-left: 20px; color: #3182ce; line-height: 1.8;">
                    ...
                    <li><a href="https://news.ycombinator.com/" style="text-decoration: none; color: #3182ce;">Hacker News (Top Systems Engineering Threads)</a></li>
                </ul>
            ...
            try:
                ...
                logging.error(f"Failed to deliver fallback pulse alert: {str(fallback_err)}")

    except Exception as pipeline_fatal:
        logging.critical(f"Catastrophic Pipeline Loop Failure: {str(pipeline_fatal)}")
    finally:
        db.close()
        logging.info("PostgreSQL context wrapper context safely released back to pool.")
