...
def run_ingestion_pipeline():
    ...
    curated_count = 0  # Initialize operational tracking metric
    ...
    try:
        ...
        print(f"❌ Core Pipeline Crash: {str(section_err)}")
    finally:
        db.close()
        print("Database connection context successfully released back to the pool.")

    # ==========================================
    # --- PHASE 3: MAIL DISPATCH BRIEFING ------
    # ==========================================
    # Early-exit evaluation: Only execute email delivery if new intelligence rows were added
    if curated_count > 0:
        ...
        try:
            ...
            print("Executive Briefing successfully dispatched to endpoint.")
        except Exception as mail_err:
            print(f"⚠️ Notification Layer Failure: {str(mail_err)}")
    else:
        print("\nPipeline Execution Halted: 0 net-new entries curated. Exiting cleanly without dispatching mail.")
